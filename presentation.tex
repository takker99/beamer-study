% aspect ratio: 4:3
\documentclass[unicode, 9pt, aspectratio=43]{beamer}
\usepackage{mybeamer}
...
% Start a reference section to track citations
\begin{refsection}
      ...
      \begin{column}{0.55\linewidth}
        \begin{block}{社会的課題}
          \begin{itemize}
            \item 軟弱地盤の改良需要が増大
            \item 建設残土の有効活用が課題
            \item 環境負荷の低減が求められる\cite{slag_stabilization}
          \end{itemize}
        \end{block}
      ...
      \begin{column}{0.48\linewidth}
        ...
        \begin{block}{鉄鋼スラグ}
          \begin{description}[粒度]
            \item[種類] 製鋼スラグ
            \item[主成分] \ce{CaO}, \ce{SiO2}, \ce{Fe2O3}
            \item[粒度] 75\(\mu \)m以下
          \end{description}
        \end{block}
      \end{column}
  ...
  \begin{frame}
    ...
    \begin{center}
      \begin{minipage}{0.85\linewidth}
        \begin{block}{反応プロセス}
          \begin{enumerate}
            \item \textbf{初期段階}：スラグからの\ce{CaO}溶出
            \item \textbf{水和反応}：\ce{Ca (OH)2}の生成
  ...
  % References
  %%%%%%%%%%%%%%%%%%%%%%%%%
  % End the reference section first
\end{refsection}

% Only show references frame if there are citations
...
\begin{frame}
  ...
  \begin{block}{供試体の作製条件}
    \begin{itemize}
      ...
      \item 締固めエネルギー：\(E_{\mathrm{c}}\) = 550 kJ/m\(^3\)
