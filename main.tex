...
% main
\begin{document}
%\pagenumbering{arabic} % uncomment if you use romannum package: or
% page numbers would be in roman numbers
% Make translator's \translate safe inside PDF strings (bookmarks).
% Define a helper and make \translate point to it inside PDF strings
% to avoid parameter-token (#) issues when passed as an argument.
\newcommand{\NoTranslate}[1]{#1}
\pdfstringdefDisableCommands{\let\translate\NoTranslate}

% If the title still overflows, insert a manual break in the title above.
\maketitle

% main text
\begin{frame}[fragile] % beamer and verb / verbatim are
  % incompatible in default
  \frametitle{main.texとmybeamer.styについて}
  このテーマはmetropolisをベースに，mybeamer.styの中でカスタマイズしたものです．

  main.texのプリアンブルで\lstinline|\usepackage{mybeamer}|としてmybeamer.styを読み込んでいます．
...
\begin{frame}[label=blockpage] % 再掲するために名前をつける
...
\begin{frame}[fragile]
  ...
  \begin{block}{このページを水平方向に，3面に分割する}
    \begin{itemize}
      ...
      \item clolumn間の余白を考慮すると，columnの幅の合計はlinewidth（100\%）よりも小さいほうが無難．
    \end{itemize}
  \end{block}
  \vspace{\baselineskip}
  ...
  \begin{columns}
    \begin{column}{.2\linewidth}
      ...
      width=\linewidth]{figs/fig_mod.jpg}
    \end{column}
...
\begin{frame}
  ...
  しかし，\emph{結局columnsを使うのが楽で安定している}ように思う．
...
\begin{frame}
  ...
  ある要素がスライドの幅多くに拡がってほしいわけではない，という場面がよくある．
  \begin{block}{}
    \centering
  ...
  \begin{center}
    ...
    \begin{itemize}
      \item 例えばitemizeは長い項目がなくても，ページの幅多くに
...
\begin{frame}
  ...
  \begin{block}{}
    \centering
    普通にblock環境をつくると，ページ幅多くになってしまう．
  \end{block}

  \begin{minipage}{.8\linewidth}
    \begin{alertblock}{center (ing)とminipage環境を組み合わせるとよい！}
      \centering
      block環境はminipage環境の幅多くに配置される．
...
\againframeNoDest{blockpage} % 1ページ目を再掲する (no duplicate PDF destination)

\section{図を並べて配置したい}
\begin{frame}
  \frametitle{tabular環境を使うと安定して図を並べられる (1)}
  図を水平方向に並べる場合，includegraphicsのoptionに幅（width）ではなく高さ（height）を指定するほうが見た目を整えやすい．
...
\begin{frame}[fragile]
  ...
  *\lstinline|\raisebox{-.5\height}{\includegraphics[height=...}|としてある．
  ...
  \begin{frame}
    ...
    \begin{itemize}
      \item このパッケージを使うと，\texttt{\string\appendix}以降のページを除いてページ番号を計算してくれる．
    ...
    \begin{itemize}
      ...
      \item frameのタイトルはframetitleコマンドを使って指定するようにしている，
        \begin{itemize}
          \item
            \lstinline|
            \begin{frame}{タイトル}|しかし記述できるが，frameにオプションを渡したいときに挙動がよくわからない．
            \item 例えばラベルをつけようとすると，ビルドできなくなる（と考えられる）．
          \end{itemize}
      \end{itemize}

      \(\blacksquare \)\  \lstinline|#特殊記号@を\texttt{\string\verb}など|で出力したい場合
      \begin{itemize}
        \item デフォルトでは，beamer内でverb / verbatimを使うことができない．
        \item frameのオプションに[fragile]を渡すことで使えるようになる．
      \end{itemize}
